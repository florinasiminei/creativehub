...
import { completeAttractionUpload, requestAttractionUploadUrls } from '@/lib/api/attractions';
...
export default function useAttractionImageUploads({
  ...
  const upload = async (attractionId: string, files: File[], startIndex = 0, tokenOverride?: string | null) => {
    ...
    try {
      ...
      const signBody = await requestAttractionUploadUrls(
        attractionId,
        prepared.map((p) => ({
          index: p.index,
          name: p.file.name,
      ...
      for (const uploadItem of uploads) {
        ...
        const { error } = await supabase.storage
          .from('listing-images')
          .uploadToSignedUrl(uploadItem.path, uploadItem.token, entry.file, {
        ...
        try {
          const completed = await completeAttractionUpload(
            attractionId,
            uploadItem.path,
